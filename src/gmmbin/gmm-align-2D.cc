	...
	try {
		...
		for (; !feature_reader.Done(); feature_reader.Next()) {
			...
			*/
			...
			std::vector< std::vector<size_t> > most_like_state_top(block_tol_num, std::vector<size_t>(state_tol_num, 0));//当某一block由某一state产生时，这一block上方的block对应的state最可能是什么
			std::vector< std::vector<size_t> > most_like_state_left(block_tol_num, std::vector<size_t>(state_tol_num, 0));//当某一block由某一state产生时，这一block左侧的block对应的state最可能是什么
